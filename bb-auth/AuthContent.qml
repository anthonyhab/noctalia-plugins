...
Item {
  // Increments to restart animations
  ...
  // --- 1. THEME & METRICS ---
  readonly property double fontScale: (typeof Settings !== "undefined" ? Settings.data.ui.fontDefaultScale : 1)
  ...
  Item {
    ...
    transform: [
      Scale {
        ...
        xScale: root.animateIn ? 1 : 0.95
        yScale: root.animateIn ? 1 : 0.95
      },
      Translate {
    ...
    ColumnLayout {
      ...
      NBox {
        ...
        color: {
          ...
          return Qt.alpha(Color.mSurfaceVariant, isDark ? 0.68 : 0.6);
        }
        border.color: {
          if (successState)
            return successColor;
          ...
          return passwordInput.activeFocus ? Qt.alpha(contextModel ? contextModel.accentColor : Color.mSecondary, 0.6) : Color.mOutline;
        }
        border.width: {
      ...
      NButton {
        ...
        buttonRadius: root.nestedRadius(root.inputRadius, authButton.width, authButton.height, 0)
        antialiasing: true
        backgroundColor: {
          if (successState)
            return successColor;
          ...
          if (hoverActive)
            return Qt.lighter(Color.mPrimary, root.isDark ? 1.15 : 1.08);

          return Color.mPrimary;
        }
        enabled: hasSession && !busyVisual && !successState && passwordInput.text.length > 0
        opacity: (authButton.enabled || successState) ? 1 : (busyVisual ? 0.85 : 0.55)
        ...
        Rectangle {
          anchors.fill: parent
          radius: authButton.buttonRadius
          color: "transparent"
          border.width: authButton.activeFocus ? 2 : 0
          border.color: contextModel ? contextModel.accentColor : Color.mSecondary
          z: 2
