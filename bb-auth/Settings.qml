...
ColumnLayout {
  ...
  ColumnLayout {
    ...
    NText {
      text: tr("settings.dialog-behavior.title", "Dialog behavior")
    ...
    NText {
      text: tr("settings.dialog-behavior.description", "Configure how the authentication dialog appears and behaves.")
      wrapMode: Text.WordWrap
  ...
  ColumnLayout {
    ...
    RowLayout {
      ...
      NText {
        text: {
          if (!(pluginMain && pluginMain.providerRegistered))
            return tr("status.disconnected", "Disconnected from auth daemon");

          if (!(pluginMain && pluginMain.providerActivityKnown))
            return tr("status.negotiating", "Negotiating with auth daemon");

          return (pluginMain && pluginMain.providerActive) || true ? tr("status.active", "Active — ready to handle requests") : tr("status.standby", "Standby — another agent is active");
        }
        color: Color.mOnSurface
        pointSize: Style.fontSizeS
        Layout.fillWidth: true
    ...
    NText {
      Layout.fillWidth: true
      text: tr("status.conflict-policy", "Conflict policy: ") + ((pluginMain && pluginMain.agentConflictMode) || "session")
      color: Color.mOnSurfaceVariant
      pointSize: Style.fontSizeS
      opacity: 0.7
      wrapMode: Text.WordWrap
  ...
  ColumnLayout {
    ...
    NText {
      text: tr("settings.about.title", "About")
      pointSize: Style.fontSizeM
      font.weight: Style.fontWeightMedium
      color: Color.mOnSurface
    }
