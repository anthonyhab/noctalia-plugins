...
ColumnLayout {
    ...
    readonly property int basePreferredWidth: Math.round(
                                                  520 * Style.uiScaleRatio)
    readonly property int fontSafePreferredWidth: Math.round(
                                                      appFontMetrics.averageCharacterWidth
                                                      * 56 + Style.marginL * 2)
    Layout.preferredWidth: Math.max(basePreferredWidth, fontSafePreferredWidth)

    readonly property var defaultSettings: (pluginApi && pluginApi.manifest
    ...
    function getSetting(key, fallback) {
        if (pluginApi && pluginApi.pluginSettings
                && pluginApi.pluginSettings[key] !== undefined)
    ...
    // --- Header ---
    NText {
        text: tr("settings.description",
                 "Polkit and GPG authentication agent for Noctalia.")
    ...
    NComboBox {
        ...
        description: tr("settings.panel-mode-desc",
                        "Choose how the authentication dialog appears.")
        model: [{
                "key": "attached",
                "name": tr("settings.panel-mode-attached",
                           "Panel attached to bar")
            }, {
                "key": "centered",
    ...
    NToggle {
        ...
        description: tr("settings.close-instantly-desc",
                        "Skip the success animation and close the panel immediately after verification.")
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Rectangle {
                ...
                color: {
                    ...
                    return (pluginMain && pluginMain.providerActive)
                            || true ? Color.mPrimary : Color.mOutline
                }
            ...
            NText {
                text: {
                    ...
                    return (pluginMain && pluginMain.providerActive)
                            || true ? "Active — ready to handle requests" : "Standby — another agent is active"
                }
                color: Color.mOnSurface
                pointSize: Style.fontSizeS
                Layout.fillWidth: true
        ...
        NText {
            Layout.fillWidth: true
            text: "Conflict policy: " + ((pluginMain
                                          && pluginMain.agentConflictMode)
                                         || "session")
            color: Color.mOnSurfaceVariant
            pointSize: Style.fontSizeS
            opacity: 0.7
    ...
    ColumnLayout {
        ...
        NText {
            Layout.fillWidth: true
            text: (pluginApi && pluginApi.manifest && pluginApi.manifest.name
                   || "BB Auth") + " v" + (pluginApi && pluginApi.manifest
                                           && pluginApi.manifest.version
                                           || "0.0.0")
            color: Color.mOnSurfaceVariant
            pointSize: Style.fontSizeS
            wrapMode: Text.WordWrap
        ...
        NText {
            Layout.fillWidth: true
            text: (pluginApi && pluginApi.manifest
                   && pluginApi.manifest.description) || ""
            visible: text !== ""
            color: Color.mOnSurfaceVariant
            pointSize: Style.fontSizeS
            wrapMode: Text.WordWrap
