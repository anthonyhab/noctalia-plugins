...
Item {
  ...
  function startThemeChange(themeName, asyncSetter, schemeApplier, callback) {
    ...
    themeSetPromise.then(function(result) {
      ...
      Qt.callLater(function() {
        if (state === ThemeOperationManager.State.Complete ||
            state === ThemeOperationManager.State.Error) {
  ...
  function stateToString(s) {
    switch (s) {
      case ThemeOperationManager.State.Idle: return "Idle"
      case ThemeOperationManager.State.Initiating: return "Initiating"
      case ThemeOperationManager.State.SettingTheme: return "SettingTheme"
      case ThemeOperationManager.State.ApplyingScheme: return "ApplyingScheme"
      case ThemeOperationManager.State.Complete: return "Complete"
      case ThemeOperationManager.State.Error: return "Error"
      default: return "Unknown"
    }
  }

  function isOperationInProgress() {
    return state !== ThemeOperationManager.State.Idle &&
           state !== ThemeOperationManager.State.Complete &&
           state !== ThemeOperationManager.State.Error
  }
}
