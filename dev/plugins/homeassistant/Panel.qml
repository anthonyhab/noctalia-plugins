...
Item {
    ...
    readonly property int contentPreferredWidth: Math.round(
                                                     340 * Style.uiScaleRatio)
    readonly property int contentPreferredHeight: Math.round(
    ...
    Timer {
        ...
        onTriggered: {
            if (mediaPositionUpdatedAt) {
                ...
                calculatedPosition = Math.min(mediaPosition + elapsed,
                                              mediaDuration)
            }
        }
    }
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    NIconButton {
                        ...
                        tooltipText: pluginApi?.tr("actions.shuffle")
                                     || "Shuffle"
                    ...
                    NIconButton {
                        ...
                        baseSize: 28
                        enabled: isConnected
                        tooltipText: pluginApi?.tr("actions.previous")
                                     || "Previous"
                    ...
                    NIconButton {
                        ...
                        baseSize: 48
                        enabled: isConnected
                        tooltipText: isPlaying ? (pluginApi?.tr("actions.pause")
                                                  || "Pause") : (pluginApi?.tr(
                                                                     "actions.play")
                                                                 || "Play")
                    ...
                    NIconButton {
                        ...
                        colorBg: repeatMode
                                 !== "off" ? Color.mSecondaryContainer : Color.mSurfaceVariant
                        tooltipText: {
                            switch (repeatMode) {
                            case "off":
                                return pluginApi?.tr("actions.repeat-off")
                                        || "Repeat: Off"
                            case "all":
                                return pluginApi?.tr("actions.repeat-all")
                                        || "Repeat: All"
                            case "one":
                                return pluginApi?.tr("actions.repeat-one")
                                        || "Repeat: One"
                            default:
                                return "Repeat"
                            }
                ...
                RowLayout {
                    ...
                    NIconButton {
                        icon: isVolumeMuted ? "volume-off" : (volumeLevel
                                                              > 0.5 ? "volume" : "volume-2")
                        baseSize: 20
                        enabled: isConnected
                        tooltipText: isVolumeMuted ? (pluginApi?.tr(
                                                          "actions.unmute")
                                                      || "Unmute") : (pluginApi?.tr(
                                                                          "actions.mute")
                                                                      || "Mute")
                ...
                NText {
                    Layout.fillWidth: true
                    visible: !isConnected
                    text: pluginApi?.tr("panel.settings-hint")
                          || "Configure connection in Settings > Plugins > Home Assistant"
                    color: Color.mOnSurfaceVariant
