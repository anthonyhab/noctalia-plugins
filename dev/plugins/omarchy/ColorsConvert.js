...
// Generate "on" color with proper contrast (4.5:1 ratio for WCAG AA)
function generateOnColor(baseColor, isDarkMode) {
  ...
  if (isBaseLight) {
    ...
    return "#1c1b1f"; // Fallback dark color
  } else {
    let testColor = "#ffffff";
    if (getContrastRatio(baseColor, testColor) >= 4.5) {
      return testColor;
    }
    return "#e6e1e5"; // Fallback light color
  }
}
...
function generateContainerColor(baseColor, isDarkMode) {
  ...
  if (isDarkMode) {
    // Make darker and slightly more saturated
